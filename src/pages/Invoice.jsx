      ...
      try {
        ...
        setTickets(
          response.data.filter(
            (invoice) => invoice.inviceStatus === props.status
          )
        );
      } catch (error) {
      ...
      const fetchOrderDetails = async () => {
        try {
          const response = await axiosInstance.get(
            `/order/getOrderDetails/${selectedTicketId}`
          );
  ...
  return (
    <>
      ...
      <div className="container-fluid mt-3">
          ...
          <div className="container-fluid">
            <div className="row">
              <div className="col-md-3">
                <div className={`card ${dark?"bg-secondary":""}`}>
                  <div className="div-top">
                    <h3 className="title">Total Sales</h3>
                    <span className="sales">
                      0<span className="indicators">0%</span>
                    </span>
                  </div>
        ...
        {tickets.length === 0 ? (
          <>
          <h5 className="text-center py-4">No graph</h5>
          </>
        ) : (
          <div>
                ...
                <div className="order-menu-wrapper">
                  ...
                  >
                    ...
                    {tickets.map((ticket, index) => (
                      <div
                        className={`nav-link ${
                          index === selectedKey ? "bg-secondary text-white" : ""
                        }`}
                        key={index}
                        onClick={() => setSelectedKeuy(index)}
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="right-part">
                            AT :{" "}
                            <span className="badge no-status">
                              {ticket.inviceId}
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="container my-4">
                    <div className="card shadow-sm p-4">
                      <div className="d-flex justify-content-between">
                        <h5>
                          TKTID:
                          {ticketDetails ? ticketDetails.uniqueQueryId : ""}
                        </h5>
                        {dateTimeStamp && (
                          <span>{formatTimestampToDate(dateTimeStamp)}</span>
                        )}
                      </div>
                          ...
                          <div className="col-10">
                            ...
                            <p className="mb-1">
                              <span className="me-3">
                                📞{" "}
                                {ticketDetails
                                  ? ticketDetails.senderMobile
                                  : ""}
                              </span>
                              <span>
                                📧{" "}
                                {ticketDetails ? ticketDetails.senderEmail : ""}
                              </span>
                            </p>
                            <p className="mb-0">
                              <strong>Billing Address : </strong>
                              {ticketDetails ? ticketDetails.senderAddress : ""}
                            </p>
                            <p className="mb-0">
                      ...
                      <div className="d-flex justify-content-end mt-3">
                        {currency && (
                          <h5>
                            Total Ammount :-{currency} {totalAmount}
                          </h5>
                        )}
                      </div>
          ...
          <div className="container-fluid">
            <div className="table-wrapper">
              ...
              <table className={`table table-bordered ${dark?"table-dark":""}`}>
                <thead>
                  <tr>
                    <th className={` ${dark?"text-secondary":""}`} >
                      S.no.
                    </th>
                    <th className={` ${dark?"text-secondary":""}`}  tabindex="0">
                      <a data-bs-toggle="modal" data-bs-target="#exampleModal">
                        Name
                      </a>
                    </th>
                    <th className={` ${dark?"text-secondary":""}`}  tabindex="0">Ticket ID</th>
                    <th className={` ${dark?"text-secondary":""}`}  tabindex="0">Type/Status</th>
                    <th className={` ${dark?"text-secondary":""}`}  tabindex="0">Contact</th>
                    <th className={` ${dark?"text-secondary":""}`}  tabindex="0">Email</th>
                    <th className={` ${dark?"text-secondary":""}`}  tabindex="0">Description/Comment</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
